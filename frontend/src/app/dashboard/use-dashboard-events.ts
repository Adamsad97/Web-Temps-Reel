...
export function useDashboardEvents({
  ...
  return useCallback((msg: { type: string; payload: unknown }) => {
    ...
    if (msg.type === 'private_message') {
      ...
      if (activeTabRef.current !== 'messages' || selectedContactRef.current?.id !== privateMessage.fromId) {
        addToast(setToasts, { title: privateMessage.fromName || 'Message', body: (privateMessage.content || '').slice(0, MESSAGE_PREVIEW_MAX_LENGTH), icon: '💬', kind: 'msg' });
      }
      return;
    }
    ...
    if (msg.type === 'group_message') {
      ...
      if (activeTabRef.current !== 'group') {
        addToast(setToasts, { title: `Canal Interne — ${groupMessage.fromName}`, body: (groupMessage.content || '').slice(0, MESSAGE_PREVIEW_MAX_LENGTH), icon: '🏛', kind: 'group' });
      }
      return;
    }
    ...
    if (['discussion_group_created','discussion_group_member_joined','discussion_group_member_left','discussion_group_member_connected','discussion_group_member_disconnected','discussion_group_message','discussion_group_typing','discussion_group_stop_typing','discussion_group_system'].includes(msg.type)) {
      ...
      if (msg.type === 'discussion_group_system') {
        ...
        if (eventType === 'joined' || eventType === 'left') {
          addToast(setToasts, { title: 'Groupe de discussion', body: eventType === 'joined' ? `${userName} vient de rejoindre la discussion` : `${userName} a quitté la discussion`, icon: '✦', kind: 'sys' });
        }
      }
  ...
  }, [onPrivateMessage, onGroupMessage, onTypingUser, onStopTypingUser, onGroupTyping, onStopGroupTyping, onDiscussionEvent, setToasts, activeTabRef, selectedContactRef]);
}
